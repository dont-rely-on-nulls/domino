...
module Make (NT : Nt.S) = struct
  ...
  class branch ~(init_bh : Nt.branch_handle) ~(init_name : string) ~(init_tip : string)
    ...
    object (self)
      ...
      method path ?(branch : string option) (fqn : Qualified_name.t) : string =
        match branch with
        | Some b ->
            "/system/branches/" ^ b ^ "/multigroups/" ^ fqn.mg ^ "/relations/" ^ fqn.name
        | None -> (
          match mode with
          | Live ->
              "/system/branches/" ^ name ^ "/multigroups/" ^ fqn.mg ^ "/relations/" ^ fqn.name
          | Detached hash ->
      ...
      method private sync_tip () =
        ...
        | Detached _ ->
            ()
        | Live -> (
          match NT.branch_target_of_handle bh with Error _ -> () | Ok h -> tip <- h )
      ...
      method private get_mg op (mg_name : string) :
        ...
        | Some mg ->
            Ok mg
        | None ->
            Error (Error.handle_error (op ^ ": multigroup not found: " ^ mg_name))
      ...
      method clear_relation ~(mg : string) ~(rel_name : string) =
        ...
        | None ->
            Error (Error.handle_error ("relation not found: " ^ rel_name))
        | Some rel ->
  ...
  let open_branch claims branch_name =
    ...
    let mgs =
      if mgs = [] then
        [(default_mg_name, new Management.Multigroup.multigroup ~name:default_mg_name)]
      else mgs
    in
