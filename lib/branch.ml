...
module Make (NT : Nt.S) = struct
  ...
  class branch
    ...
    ~(init_name : string)
    ~(init_tip  : string)
    ~(init_mgs  : (string * Management.Multigroup.multigroup) list)
    ~(init_mode : mode) =
  object (self)
    ...
    method mg_of (n : string) : Management.Multigroup.multigroup option =
      Hashtbl.find_opt mgs n
    ...
    method path ?(branch : string option) (fqn : Qualified_name.t) : string =
      match branch with
      | Some b ->
          "/system/branches/" ^ b ^ "/multigroups/" ^ fqn.mg
          ^ "/relations/" ^ fqn.name
      | None ->
          match mode with
          | Live ->
              "/system/branches/" ^ name ^ "/multigroups/" ^ fqn.mg
              ^ "/relations/" ^ fqn.name
          | Detached hash ->
              "/system/snapshots/" ^ hash ^ "/relations/" ^ fqn.name
      ...
      | Some rel ->
          let* (bh', mg') =
            NT.clear_relation bh mg_val
              ~branch_name:name ~mg_name:mg rel
          in
          bh <- bh'; Hashtbl.replace mgs mg mg'; self#sync_tip (); Ok ()
    ...
    method register_domain ~(mg : string) domain =
      ...
      bh <- bh'; Hashtbl.replace mgs mg mg'; Ok ()

    method create_immutable_relation ~(mg : string) ~name:rel_name ~schema
        ~generator ~membership_criteria ~cardinality ~producer =
      ...
      in
      bh <- bh'; Hashtbl.replace mgs mg mg'; Ok rel

    method register_constraint ~(mg : string) ~constraint_name ~relation_name
        ~body =
      let* () = self#assert_live "register_constraint" in
      let* mg_val = self#get_mg "register_constraint" mg in
      let* (bh', mg') =
        NT.register_constraint bh mg_val ~constraint_name ~relation_name ~body
      in
      bh <- bh'; Hashtbl.replace mgs mg mg'; Ok ()
    ...
    method insert_tuple ~(mg : string) ~(rel_name : string) attrs =
      ...
      let* hash =
        NT.insert_tuple ~branch_name:name ~mg_name:mg ~rel_name attrs
      in
    ...
    method create_tuple ~(mg : string) ~(rel_name : string) tuple =
      ...
      let* hash =
        NT.create_tuple ~branch_name:name ~mg_name:mg ~rel_name tuple
      in
      self#sync_tip (); Ok hash
    ...
    method create_tuples ~(mg : string) ~(rel_name : string) tuples =
      ...
      let* hash =
        NT.create_tuples ~branch_name:name ~mg_name:mg ~rel_name tuples
      in
      self#sync_tip (); Ok hash
    ...
    method retract_tuple ~(mg : string) ~(rel_name : string) ~tuple_hash =
      ...
      let* () =
        NT.retract_tuple ~branch_name:name ~mg_name:mg ~rel_name tuple_hash
      in
      self#sync_tip (); Ok ()
    ...
    method apply_delta (delta : (string * Management.Multigroup.multigroup) list) =
      List.iter
        (fun (mg_name, mg_val) -> Hashtbl.replace mgs mg_name mg_val)
        delta;
      self#sync_tip ()
  ...
  let open_branch claims branch_name =
    ...
    let tip = match NT.branch_target_of_handle bh with
      | Ok h    -> h
      | Error _ -> ""
    in
      ...
      List.map
        (fun (mg_name, _mg_hash) ->
          (* For each bound mg, enumerate its relations from the kernel and
             ...
             reads later. *)
          let rels =
            match NT.list_relations ~branch_name ~mg_name with
            | Ok pairs -> pairs
            | Error _  -> []
          in
          let mg =
    ...
    let mgs =
      if mgs = [] then
        [ (default_mg_name,
           new Management.Multigroup.multigroup ~name:default_mg_name) ]
      else mgs
    in
    Ok (new branch
          ~init_bh:bh ~init_name:branch_name ~init_tip:tip ~init_mgs:mgs
          ~init_mode:Live)
