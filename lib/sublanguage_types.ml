type cursor_result = {cursor_id: string; rows: Tuple.materialized list; has_more: bool}

(** A Transition delta names every multigroup that the statement touched and the
    updated value for each. Most statements yield a single-element list;
