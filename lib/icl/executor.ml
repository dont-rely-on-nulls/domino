module Make (NT : Nt.S) = struct
  ...
  let convert_binding_expr : Ast.binding_expr -> Constraint.binding_expr = function
    | Ast.Var name ->
        Constraint.Var name
    | Ast.Const value ->
        Constraint.Const (Drl.Ast.value_to_abstract value)

  let convert_binding (pairs : (string * Ast.binding_expr) list) : Constraint.binding =
    List.fold_left
      (fun acc (key, expr) -> Constraint.BindingMap.add key (convert_binding_expr expr) acc)
  ...
  let rec convert_body : Ast.constraint_body -> Constraint.t = function
    ...
    | Ast.And bodies ->
        Constraint.and_ (List.map convert_body bodies)
    | Ast.Or bodies ->
        Constraint.or_ (List.map convert_body bodies)
  ...
  let execute (ctx : Sublanguage_context.t) (stmt : Ast.statement) :
    ...
    | Ast.RegisterConstraint {constraint_name; relation_name; body} ->
        ...
        let* _bh, new_mg =
          NT.register_constraint ctx.write_handle mg ~constraint_name ~relation_name:fqn.name
            ~body:runtime_body
        in
