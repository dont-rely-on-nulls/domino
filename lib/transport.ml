...
module TCP : TRANSPORT = struct
  type configuration = {address_family: Unix.socket_domain; sockaddr: Unix.sockaddr}
  ...
  let parse sexp =
    ...
    let rec go addr port path = function
      | [] -> (
        ...
        | Some a, Some p, None ->
            sockaddr_of_fields (`Inet (a, p))
        | None, None, Some s ->
            sockaddr_of_fields (`Unix s)
        | None, None, None ->
            errorf "transport/tcp: no address fields"
        | _ ->
            errorf "transport/tcp: use (address + port) or (path), not both" )
      | List [Atom "address"; Atom a] :: rest ->
          go (Some a) port path rest
      | List [Atom "port"; Atom p] :: rest -> (
        match int_of_string_opt p with
        | Some n ->
            go addr (Some n) path rest
        | None ->
            errorf "transport/tcp: invalid port: %s" p )
      | List [Atom "path"; Atom p] :: rest ->
          go addr port (Some p) rest
      | bad :: _ ->
          errorf "transport/tcp: unexpected: %s" (to_string bad)
    in
    match sexp with
    | List children ->
        go None None None children |> Result.map_error Error.parse_error
    | _ ->
        Error (Error.parse_error "transport/tcp: expected a list of fields")
