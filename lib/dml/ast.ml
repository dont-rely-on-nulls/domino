...
type statement =
  ...
  | DeleteWhere of {target: string; predicate: Drl.Ast.query}
      (** Binds an unqualified name to a session-scoped ephemeral relation. The
          body is recomputed on each scan and never materialized; the binding
          lives until [DropDefine] or a session close. In contrast, [Assign]
          drains the body into an existing stored relation. *)
