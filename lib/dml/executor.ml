module Make (NT : Nt.S) = struct
  ...
  module Error = struct
    ...
    let multigroup_not_found name =
      condition "multigroup-not-found" "Multigroup not found" ("name" |=| of_string name)
  end
  ...
  let build_tuple ~relation (attributes : Ast.attr_value list) : Tuple.materialized =
    let attr_map =
      List.fold_left
        (fun acc (name, value) ->
          Tuple.AttributeMap.add name {Attribute.value= Drl.Ast.value_to_abstract value} acc )
  ...
  let rec base_dep_paths (ctx : Sublanguage_context.t) (q : Drl.Ast.query) : string list =
    match q with
    | Drl.Ast.Base name -> (
  ...
  let tuple_matches_pred common (target_t : Tuple.materialized) (pred_t : Tuple.materialized) =
    List.for_all
      (fun attr ->
        match
  ...
  let semijoin common target_tuples pred_tuples =
    List.filter (fun t -> List.exists (tuple_matches_pred common t) pred_tuples) target_tuples
  ...
  let execute (ctx : Sublanguage_context.t) (stmt : Ast.statement) :
    ...
    | Ast.InsertTuple {relation; attributes} ->
        ...
        let* _ = NT.create_tuple ~branch_name ~mg_name:fqn.mg ~rel_name:rel#name tuple in
        after fqn
    | Ast.InsertTuples {relation; tuples} ->
        let* fqn = parse_fqn relation in
        let* rel = get_rel ctx fqn in
        let tuple_list = List.map (build_tuple ~relation:rel#name) tuples in
        let* _ = NT.create_tuples ~branch_name ~mg_name:fqn.mg ~rel_name:rel#name tuple_list in
        after fqn
    | Ast.DeleteTuple {relation; attributes} ->
        ...
        let* () = NT.retract_tuple ~branch_name ~mg_name:fqn.mg ~rel_name:rel#name hash in
        after fqn
    ...
    | Ast.Define {target; body} ->
        let* () =
          if String.contains target ':' then
            Error
              (Error.parse_error ("Define target must be unqualified (session-scoped): " ^ target))
          else Ok ()
        in
        ...
        let* _path =
          NT.register_ephemeral ~session:ctx.session_hash ~named:true ~name:target ~generator
            ...
            ~identity:(Drl.Parser.to_string body) ~schema ~deps:(base_dep_paths ctx body)
        in
        Ok []
    | Ast.DropDefine {target} ->
        let* () = NT.drop_ephemeral ~session:ctx.session_hash ~name:target in
        Ok []
