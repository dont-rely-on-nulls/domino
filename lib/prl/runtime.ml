  ...
  let rec go acc = function
    ...
    | (attr_name, _) :: rest -> (
      ...
      | None ->
          None
      | Some attr ->
          go ((attr_name, attr.Attribute.value) :: acc) rest )
  in
  go [] schema
  ...
  let rec go acc = function
    ...
    | (attr_name, _) :: rest -> (
      ...
      | None ->
          None
      | Some value ->
          go (Tuple.AttributeMap.add attr_name {Attribute.value} acc) rest )
  in
...
let rows_to_generator relation_name schema (rows : Sakura_prl_api.tuple list) : Generator.t =
  ...
  let rec gen i _pos =
    if i >= n then Generator.Done
      ...
      | None ->
          Generator.Error "Plugin row does not conform to predicate schema."
      | Some tuple ->
          Generator.Value (Tuple.Materialized tuple, gen (i + 1))
  in
  gen 0
  ...
  | Some produce -> (
      ...
      fun _pos ->
        ...
        | Error e ->
            Generator.Error e
        | Ok rows ->
            rows_to_generator relation_name schema rows _pos )
