...
let catalog_relation_names =
  [ relation_rel_name; domain_rel_name; attribute_rel_name; constraint_rel_name; on_rel_name;
    timing_rel_name; schema_rel_name; function_predicate_rel_name; loaded_library_rel_name ]

(** Relations exclusive to the sakura meta-multigroup. *)
let sakura_only_relation_names = [multigroup_rel_name]

let all_catalog_relation_names = catalog_relation_names @ sakura_only_relation_names
...
let attribute_schema : Schema.t =
  ...
  |> Schema.add "attr_name" "string" |> Schema.add "domain_name" "string"

let constraint_schema : Schema.t =
  Schema.empty |> Schema.add "name" "string" |> Schema.add "relation_name" "string"
...
let function_predicate_schema : Schema.t =
  Schema.empty |> Schema.add "name" "string" |> Schema.add "symbol" "string"
  |> Schema.add "cardinality" "string" |> Schema.add "purity" "string"

let loaded_library_schema : Schema.t = Schema.empty |> Schema.add "path" "string"

(** Definitions seeded into every multigroup. *)
let catalog_definitions =
  [ (relation_rel_name, relation_schema); (domain_rel_name, domain_schema);
    (attribute_rel_name, attribute_schema); (constraint_rel_name, constraint_schema);
    (on_rel_name, on_schema); (timing_rel_name, timing_schema); (schema_rel_name, schema_schema);
...
let build_relation_tuple rel_name : Tuple.materialized =
  { Tuple.relation= relation_rel_name;
    attributes= Tuple.AttributeMap.singleton "name" {Attribute.value= Obj.magic rel_name} }

let build_domain_tuple dom_name : Tuple.materialized =
  { Tuple.relation= domain_rel_name;
    attributes= Tuple.AttributeMap.singleton "name" {Attribute.value= Obj.magic dom_name} }
...
let build_on_tuple event : Tuple.materialized =
  { Tuple.relation= on_rel_name;
    attributes= Tuple.AttributeMap.singleton "event" {Attribute.value= Obj.magic event} }

let build_timing_tuple timing : Tuple.materialized =
  { Tuple.relation= timing_rel_name;
    attributes= Tuple.AttributeMap.singleton "timing" {Attribute.value= Obj.magic timing} }

let build_multigroup_tuple mg_name : Tuple.materialized =
  { Tuple.relation= multigroup_rel_name;
    attributes= Tuple.AttributeMap.singleton "name" {Attribute.value= Obj.magic mg_name} }

let build_schema_tuple schema_name : Tuple.materialized =
  { Tuple.relation= schema_rel_name;
    attributes= Tuple.AttributeMap.singleton "name" {Attribute.value= Obj.magic schema_name} }
...
let cardinality_to_string c = Sexplib.Sexp.to_string (Conventions.Cardinality.sexp_of_t c)
...
let build_function_predicate_tuple ~name ~symbol ~cardinality ~purity : Tuple.materialized =
  { Tuple.relation= function_predicate_rel_name;
    attributes=
      Tuple.AttributeMap.of_list
        [ ("name", {Attribute.value= Obj.magic name});
          ("symbol", {Attribute.value= Obj.magic symbol});
          ("cardinality", {Attribute.value= Obj.magic (cardinality_to_string cardinality)});
...
let build_loaded_library_tuple path : Tuple.materialized =
  { Tuple.relation= loaded_library_rel_name;
    attributes= Tuple.AttributeMap.singleton "path" {Attribute.value= Obj.magic path} }
