...
module MemoryBackend : BACKEND with type configuration = unit = struct
  ...
  module Error = struct
    ...
    let already_in_transaction =
      condition "already-in-transaction" "Already inside a transaction" empty
  ...
  let get_many conn hashes = Ok (List.map (fun h -> Hashtbl.find_opt conn.data h) hashes)
