...
class multigroup ~name:(init_name : Conventions.Name.t) =
  object (_self)
    ...
    method remove_relation rel_name = {<relations = BatMap.String.remove rel_name relations>}
    ...
    method serialize () : Storable.Multigroup.t =
      { Storable.Multigroup.name;
        relations=
          BatMap.String.bindings relations |> List.map snd |> List.map (fun r -> r#serialize ()) }
  end
    ...
    let db =
      List.fold_left
        (fun mg rel ->
          let relation =
            new Relation.stored
              ~name:rel.Storable.Relation.name ~schema:rel.schema ~constraints:None
