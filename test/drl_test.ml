...
let seed_fixture (branch : Test_Branch.branch) (multigroup : Management.Multigroup.multigroup) :
  ...
  fixture |> List.map create |> List.fold_left sequencing (Ok ()) |> Result.map ignore

let make_fixture () =
  let* _ = Test_Nt.initialize () in
  (* FIXME: this *leaks* on the C++ side! *)
...
let ensure_contains t ts =
  ...
  Result.bind ts (fun ts ->
      if contains tuple ts then Ok ts
